...
def FindGainedPrivileges(SP, CP, TSA):
    ...
    if CP.CPEID == SP.Preconditions[0].CPEID:
...
def FormPrivileges(preConditions, CP, TSA):
    ...
    for i in TSA.InformationSources:
        if (i.Preconditions[0].CPEID == CP.CPEID) :
            return [CP]
    for i in TSA.Vulnerabilities:
        if CP.CPEID == i.Preconditions[0].CPEID:
            return [CP]
    return []
...
# 绑定端口5000, 设置验证码'abc':
manager = QueueManager(address=('10.52.182.83', 5000), authkey=b'abc')
# 启动Queue:
manager.start()
# 获得通过网络访问的Queue对象:
...
if __name__ == '__main__':
    Condition_1 = Condition('File Access', 'Host 1 Windows XP', 'cpe:/10:microsoft:windows_xp::sp2','88.132.3.24')
    Condition_2 = Condition('File Access', 'Host 2 Internet Explorer', 'cpe:/a:microsoft:iternet_explorer:10', '75.62.2.22')
    Condition_3 = Condition('Memory Access', 'Host 2 Internet Explorer', 'cpe:/a:microsoft:iternet_explorer:10', '75.62.2.22')
    Condition_4 = Condition('File Access', 'Host 2 Mozilla Thunderbird', 'cpe:/a:mozilla:thunderbird:17.0.2', '75.62.2.22')
    Condition_5 = Condition('Authorization', 'Host 4 Apache Web Server', 'cpe:/a:apache:http_server:2.2.4', '75.62.3.33')
    Condition_6 = Condition('User Right', 'Host 3 Windows XP', 'cpe:/o:microsoft:windows_xp::sp2', '75.62.3.35')

    Vulnerability_1 = Vulnerability('CVE-2010-3004',[Condition_1],[Condition_2,Condition_3])
    Vulnerability_2 = Vulnerability('CVE-2011-3544', [Condition_1], [Condition_4])
    Vulnerability_3 = Vulnerability('CVE-2012-4576', [Condition_3], [Condition_6])

    InformationSource_0 = InformationSource('Credentials Store X',['Host 2 Internet Explorer'],[Condition_2],[Condition_5])

    SoftwareApplication_1 = SoftwareApplication('cpe:/10:microsoft:windows_xp::sp2','88.132.3.24',0,['Host 1 Windows XP'],[],[])
    SoftwareApplication_2 = SoftwareApplication('cpe:/a:microsoft:iternet_explorer:10', '75.62.2.22', 0, ['Host 2 Internet Explorer'], [], [Vulnerability_1])
    SoftwareApplication_3 = SoftwareApplication('cpe:/a:mozilla:thunderbird:17.0.2', '75.62.2.22', 1, ['Host 2 Mozilla Thunderbird'], [], [Vulnerability_2])
    SoftwareApplication_4 = SoftwareApplication('cpe:/a:apache:http_server:2.2.4', '75.62.3.33', 0, ['Host 4 Apache Web Server'], [InformationSource_0], [])
    SoftwareApplication_5 = SoftwareApplication('cpe:/o:microsoft:windows_xp::sp2', '75.62.3.35', 0,['Host 3 Windows XP'], [], [Vulnerability_3])

    Organization = NetworkHost('88.132.3.24',[],[SoftwareApplication_1])  # origin attack
    DMZ = NetworkHost('75.62.2.22',[],[SoftwareApplication_2,SoftwareApplication_3])  # 4 web servers    &&    2 Database    &&    1 Mail
    ALAN = NetworkHost('75.62.3.33',[],[SoftwareApplication_4])
    LAN1 = NetworkHost('75.62.3.35',[],[SoftwareApplication_5])

    TargetNetwork = HyperGraph()
    TargetNetwork.Node = [Organization, DMZ, ALAN, LAN1]
    TargetNetwork.Edge = [[Organization, DMZ], [DMZ, ALAN], [DMZ, LAN1]]
    ...
    task.put(TargetNetwork)
    task.put(IPRGS)

    foundPrivileges = result.get(timeout=60)
    partialAttackGraph = result.get(timeout=60)
    ...
    for i in partialAttackGraph.Node:
        A.add_node(i.sping())
    for i in partialAttackGraph.Edge:
        A.add_edge(i[0].sping(),i[1].sping())

    A.write('AttackGraph_f.dot')
    A.layout('dot')  # layout with dot
    A.draw('AttackGraph_f.png')  # write to file
